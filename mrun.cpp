...
void OnOff_Test(Parameter para, Packet*P, Result*R, set<Data>APT_S =  emptyset){
    R->para = para;
...
void PIE_Test(Parameter para, Packet*P, Result*R, set<Data>APT_S = emptyset){
    R->para = para;
    uint64_t input_max = para.input_max;
    uint64_t max_mem   = para.max_mem;
    int threshold      = para.threshold;
...
void PI_Test(Parameter para, Packet*P, Result*R, set<Data> APT_S = emptyset){
    R->para = para;
    uint64_t input_max = para.input_max;
    uint64_t max_mem   = para.max_mem;
    int threshold      = para.threshold;
...
void STD_Test(Parameter para, Packet*P, Result*R, set<Data> APT_S = emptyset){
    R->para = para;
    uint64_t input_max = para.input_max;
    uint64_t max_mem   = para.max_mem;
    int threshold      = para.threshold;
...
void work(const char* filename,const char* output){
    ...
    for(int i=0; i< MAX_INPUT_SIZE && ExtractFormattedPacket(f,atP); ++i){
        ...
        if(i%1000000 == 0) cerr << atP->us <<" "<< i<< endl;
    }
...
*/
    ...
    for(uint64_t mem = 100*1024; mem <= 300*1024; mem += 100*1024){
        ...
        for(int para_p = 1; para_p <= 10; ++ para_p){
            PI_Test({5000000,mem,initThres,5000,initL,para_p},P,&R,atkS);
            PI_THP ({5000000,mem,initThres,5000,initL,para_p},P,&R);
            R.Print_p();      
        }
    }
    ...
    for(uint64_t mem = 100*1024; mem <= 300*1024; mem += 100*1024){
        ...
        for(uint64_t Nt = 1*1000000; Nt <= 8*1000000;  Nt += 1000000){
            PI_Test({Nt,mem,initThres * Nt / 5000000 ,5000,initL,5},P,&R,atkS);
            PI_THP ({Nt,mem,initThres * Nt / 5000000 ,5000,initL,5},P,&R);
            R.Print_Nt();              
        }
    }
    ...
    for(uint64_t mem = 100*1024; mem <= 300*1024; mem += 100*1024){
        ...
        for(int ths = 1000; ths <= 5000; ths += 1000){
            PI_Test({5000000,mem,initThres ,5000,initL,5},P,&R,atkS);
            PI_THP ({5000000,mem,initThres ,5000,initL,5},P,&R);
            R.Print_Npi();         
        }
    }
    ...
    printf("OO\nMEM,ARE,PR,RR,F1,THP\n");
    for(uint64_t mem = 50*1024; mem <= 300*1024; mem += 50*1024){
        OnOff_Test({5000000,mem,initThres,5000,initL,5},P,&R,atkS);
        OnOff_THP ({5000000,mem,initThres,5000,initL,5},P,&R);
#if APT
        R.Print_APT();
#else
        R.Print();
    ...
    for(uint64_t mem = 100*50*1024; mem <= 100*300*1024; mem += 100*50*1024){
        PIE_Test({5000000,mem,initThres,5000,initL,5},P,&R,atkS);
        PIE_THP ({5000000,mem,initThres,5000,initL,5},P,&R);
#if APT
        R.Print_APT();
#else
        R.Print();
...
#if APT
    printf("STD\nMEM,ARE,PR,RR,F1,THP\n");
    STD_Test({5000000,50,initThres,5000,initL,5},P,&R,atkS);
    R.Print_APT();
#endif
    // cerr << "PI" << endl;
